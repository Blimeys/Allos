...
export const fetchMenusCategories = currentPath => async dispatch => {
	const res = await axios.get('/api/public/' + currentPath);
	let categories = [];
	if (res.data.error === true) {
		dispatch({ type: MENUS_CATEGORIES, payload: categories });
	} else {
		res.data.map(item => {
			return categories.push(item.category.toLowerCase());
		});
...
export const toggleResultsView = value => async dispatch => {
	if (!value.showResults) {
		value.showResults = true;
	} else {
		value.showResults = false;
	}
	console.log(value);
	dispatch({ type: TOGGLE_RESULTS, payload: value });
};
